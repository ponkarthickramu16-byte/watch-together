...
function Room() {
    ...
    // FIX 4: Direct document lookup instead of query - more reliable and faster
    const roomLoadedRef = useRef(false);
    useEffect(() => {
        console.log("[Watch Together] Setting up room listener for:", roomId);

        // Use direct document reference instead of query for better reliability
            ...
            (snapshot) => {
                if (snapshot.exists()) {
                    ...
                    console.log("[Watch Together Debug] Room data:", {
                        ...
                        movieUrl,
                        movieUrlType: typeof movieUrl,
                        movieUrlLength: movieUrl?.length,
                    ...
                    if (videoRef.current && data.currentTime !== undefined && !isSyncingSeekRef.current) {
                        ...
                        if (diff > 0.8) videoRef.current.currentTime = data.currentTime;
                    }
                    ...
                    console.error("[Watch Together] Room document not found:", roomId);
                    showToast("Room இல்ல! இந்த Room ID தவறா இருக்கு.", "❌", "#e74c3c");
...
if (!roomLoadedRef.current && movieUrl) {
    ...
    roomLoadedRef.current = true;
    showToast("Room loaded without a movie URL.", "⚠️", "#f39c12");
}

if (!isSyncingRef.current) setIsPlaying(normalizedData.isPlaying);
if (videoRef.current && normalizedData.currentTime !== undefined && !isSyncingSeekRef.current) {
    ...
    if (diff > 0.8) videoRef.current.currentTime = normalizedData.currentTime;
}
if (videoRef.current && !isSyncingRef.current) {
    if (normalizedData.isPlaying && videoRef.current.paused) videoRef.current.play().catch(() => { });
    else if (!normalizedData.isPlaying && !videoRef.current.paused) videoRef.current.pause();
}
            } else {
